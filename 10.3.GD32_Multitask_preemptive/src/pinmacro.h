...
#define GPIO_mode(port, bit, mode)\
  do{ \
    if(bit<8)_GPIO_CONFIG(port,bit,GPIO_CTL0(GPIO(port)),mode); \
      else _GPIO_CONFIG(port,bit-8,GPIO_CTL1(GPIO(port)),mode); \
  }while(0)
...
#define GPIO_config(descr) \
  do{ \
    ...
    if(marg4(descr) == GPIO_PULL){ \
      GPIO_BOP(GPIO(marg1(descr))) = (1<<marg2(descr)<<((marg3(descr))*16)); \
    } \
  }while(0)
...
#define GPO_ON(descr) \
  do{ \
    GPIO_BOP(GPIO(marg1(descr))) = (1<<marg2(descr)<<((1-marg3(descr))*16)); \
  }while(0)
...
#define GPO_T(descr) \
  do{ \
    GPIO_OCTL(GPIO(marg1(descr))) ^= (1<<marg2(descr)); \
  }while(0)
#define GPI_ON(descr)  ((GPIO_ISTAT(GPIO(marg1(descr))) & (1<<marg2(descr)))==(marg3(descr)<<marg2(descr)))
#define GPI_OFF(descr) ((GPIO_ISTAT(GPIO(marg1(descr))) & (1<<marg2(descr)))!=(marg3(descr)<<marg2(descr)))
