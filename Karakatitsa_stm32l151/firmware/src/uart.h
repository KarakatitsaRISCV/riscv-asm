...
#define UART_speed(num, divider) do{ \
  if(num == 1){\
    RCC->APB2ENR |= RCC_APB2ENR_USART1EN;\
    USART1->BRR = 32000000UL/(divider); \
  }else{\
    if(num == 2)RCC->APB1ENR |= RCC_APB1ENR_USART2EN;\
    if(num == 3)RCC->APB1ENR |= RCC_APB1ENR_USART3EN;\
    UART(num)->BRR = 32000000UL/(divider);\
  }\
}while(0)

#define UART_init(num, divider) do{\
  NVIC_DisableIRQ( UART_IRQ(num) );\
  GPIO_config( UART_PIN(num,RX) ); \
  GPIO_config( UART_PIN(num,TX) ); \
  UART_speed(num, divider); \
...
//***********************************************************************************************
//***********************************************************************************************
#define UART1_TX A,9 ,1,(GPIO_ALT_VS | AF_UART1)
#define UART1_RX A,10,1,(GPIO_ALT_VS | AF_UART1)

#define UART2_TX A,2 ,1,(GPIO_ALT_VS | AF_UART2)
#define UART2_RX A,3 ,1,(GPIO_ALT_VS | AF_UART2)

#define UART3_TX B,10,1,(GPIO_ALT_VS | AF_UART3)
#define UART3_RX B,11,1,(GPIO_ALT_VS | AF_UART3)

#if 0
UART1 (TX / RX / CK / CTS / RTS)
  A9 / A10 / A8 / A11 / A12
  B6 / B7
UART2
  A2 / A3 / A4 / A0 / A1
